...
OFFSET = 0
AJUSTE_MOTOR = 40
LISTA_CORES_PISTA = [ColorSensor.COLOR_WHITE, ColorSensor.COLOR_RED, ColorSensor.COLOR_BLUE, ColorSensor.COLOR_GREEN,
                     ColorSensor.COLOR_BLACK]
...
def achou_cor(cor, valor_cor, indice_cor=None):
    ...
    if cor == "PRETO":
        ...
        funcao_motores.acelerar_individual(-1 * (VELOCIDADE_ATUAL - 200), -1 * (VELOCIDADE_ATUAL - 200 + AJUSTE_MOTOR),
                                           (fim * 1000) - 200)
        direita_giroscopio(ANGULO_CURVA_90)
    else:
        if sentido == "INDO":
...
def chegou_final():
    ...
    global flag_parar, final, quant_idas, sentido, angulo_atual, boneco_dir
    ...
    if quant_idas % 2 == 0:
        ...
        direita_giroscopio(ANGULO_CURVA_90)
        funcao_motores.acelerar_individual((VELOCIDADE_ATUAL + 200), (VELOCIDADE_ATUAL + AJUSTE_MOTOR + 200), 2500)
...
def eh_rampa():
    ...
    global quant_bonecos, final, sentido, angulo_atual
...
funcao_motores.acelerar_porta(-1 * VELOCIDADE_ATUAL)
    ...
    if lado == "DIREITA":
        direita_giroscopio(5)
        funcao_motores.acelerar_individual(VELOCIDADE_ATUAL, VELOCIDADE_ATUAL + AJUSTE_MOTOR, 1400)
        direita_giroscopio(40)
    else:
        esquerda_giroscopio(5)
        funcao_motores.acelerar_individual(VELOCIDADE_ATUAL, VELOCIDADE_ATUAL + AJUSTE_MOTOR, 1400)
        esquerda_giroscopio(40)
    ...
    if sentido == "DIREITA":
        direita_giroscopio(5)
        funcao_motores.acelerar_individual(-1 * VELOCIDADE_ATUAL, -1 * (VELOCIDADE_ATUAL + AJUSTE_MOTOR), 500)
        direita_giroscopio(25)
    else:
        esquerda_giroscopio(5)
        funcao_motores.acelerar_individual(-1 * VELOCIDADE_ATUAL, -1 * (VELOCIDADE_ATUAL + AJUSTE_MOTOR), 500)
        esquerda_giroscopio(25)
