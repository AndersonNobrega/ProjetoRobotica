...
def controle_proporcional_rampa():
    ...
    velocidade_nova_dir = velocidade_referencia - correcao
    velocidade_nova_esq = (velocidade_referencia + correcao) + AJUSTE_MOTOR
...
def direita_giroscopio(angulo_curva):
    ...
    while True:
        funcao_motores.fazer_curva_dir_esteira(VELOCIDADE_CURVA - 150)
...
def esquerda_giroscopio(angulo_curva):
    ...
    while True:
        funcao_motores.fazer_curva_esq_esteira(VELOCIDADE_CURVA - 150)
        if ang_atual - angulo_curva >= GYRO.value():
            parar_motor()
            break
...
def procura_cor(valor_cor):
    while True:
        if cor_atual1 != valor_cor:
            funcao_motores.acelerar_dir(-1 * (VELOCIDADE_ATUAL - 350))
        else:
            funcao_motores.acelerar_dir(0)

        if cor_atual2 != valor_cor:
            funcao_motores.acelerar_esq(-1 * (VELOCIDADE_ATUAL + AJUSTE_MOTOR - 350))
        else:
...
def ajustar_na_cor(valor_cor):
    ...
    while True:
        if cor_atual1 == valor_cor:
            funcao_motores.acelerar_dir(-1 * (VELOCIDADE_ATUAL - 250))
        else:
            funcao_motores.acelerar_dir(0)

        if cor_atual2 == valor_cor:
            funcao_motores.acelerar_esq(-1 * (VELOCIDADE_ATUAL + AJUSTE_MOTOR - 250))
        else:
            funcao_motores.acelerar_esq(0)
...
def ajustar_dentro_cor(valor_cor):
    ...
    while True:
        if cor_atual1 == valor_cor:
            funcao_motores.acelerar_dir(VELOCIDADE_ATUAL - 260)
        else:
            funcao_motores.acelerar_dir(0)

        if cor_atual2 == valor_cor:
            funcao_motores.acelerar_esq((VELOCIDADE_ATUAL + AJUSTE_MOTOR) - 260)
        else:
            funcao_motores.acelerar_esq(0)
...
def achou_cor(cor, valor_cor, indice_cor=None):
    ...
    """
    global angulo_atual, boneco_esq, boneco_dir

    funcao_motores.acelerar_individual(VELOCIDADE_ATUAL, VELOCIDADE_ATUAL + AJUSTE_MOTOR, 250)
    ...
    if cor == "PRETO":
        ...
        if tempo < 0:
            tempo = 700
        funcao_motores.acelerar_individual(-1 * (VELOCIDADE_ATUAL - 285), -1 * (VELOCIDADE_ATUAL - 285 + AJUSTE_MOTOR),
                                           tempo)
        direita_giroscopio(ANGULO_CURVA_90)
    else:
        if aprender_caminho():
...
def reseta_variaveis():
    ...
    global quant_bonecos, quant_idas, sentido, cor_atual1, cor_atual2
...
def chegou_final():
    ...
    global flag_parar, final, quant_idas, sentido, angulo_atual, boneco_dir, boneco_esq, quant_bonecos

    funcao_motores.acelerar_individual(VELOCIDADE_ATUAL, VELOCIDADE_ATUAL + AJUSTE_MOTOR, 2800)
    ...
    direita_giroscopio(ANGULO_CURVA_90)
    funcao_motores.acelerar_individual((VELOCIDADE_ATUAL + 200), (VELOCIDADE_ATUAL + AJUSTE_MOTOR + 200), 4800)
    funcao_motores.acelerar_individual(-1 * VELOCIDADE_ATUAL, -1 * (VELOCIDADE_ATUAL + AJUSTE_MOTOR), 500)

    direita_giroscopio(ANGULO_CURVA_90)
    funcao_motores.acelerar_individual((VELOCIDADE_ATUAL + 200), (VELOCIDADE_ATUAL + AJUSTE_MOTOR + 200), 2900)
    ...
    while True:
        ...
        if angulo_atual - 85 >= GYRO.value():
            funcao_motores.acelerar_individual(VELOCIDADE_ATUAL - 100, VELOCIDADE_ATUAL + AJUSTE_MOTOR - 100, 2500)
            break
    ...
    reseta_pid(2.5, 0.7, 0)

    funcao_motores.acelerar_individual(VELOCIDADE_ATUAL, VELOCIDADE_ATUAL + AJUSTE_MOTOR, 550)
    angulo_atual = GYRO.value()
...
def eh_rampa():
    ...
    global quant_bonecos, final, sentido, angulo_atual, boneco_dir, boneco_esq
    ...
    funcao_motores.acelerar(0, 300)
    funcao_motores.acelerar(VELOCIDADE_ATUAL, 200)
    inicio = time()

    funcao_motores.acelerar_individual(VELOCIDADE_ATUAL - 285, VELOCIDADE_ATUAL + AJUSTE_MOTOR - 285, 700)
    ...
    if cor_atual1 == ColorSensor.COLOR_RED and cor_atual1 == ColorSensor.COLOR_RED:
        ...
        funcao_motores.acelerar_individual(-1 * (VELOCIDADE_ATUAL - 250), -1 * (VELOCIDADE_ATUAL + AJUSTE_MOTOR - 250),
                                           tempo)
        funcao_motores.acelerar(0, 300)
        return
    ...
    funcao_motores.acelerar_individual(VELOCIDADE_ATUAL, VELOCIDADE_ATUAL + AJUSTE_MOTOR, 4600)
...
def percurso():
    ...
    global sentido, quant_idas, quant_bonecos, boneco_esq, boneco_dir, angulo_atual, cor_atual1, cor_atual2
...
def main():
    global OFFSET, angulo_atual, boneco_dir, boneco_esq
