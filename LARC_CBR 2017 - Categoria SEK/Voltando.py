...
def chegou_final():
    # Chegou no circulo preto na ultima plataforma
    global final, quant_bonecos, sentido

    acelerar(VELOCIDADE_ATUAL, 1400)
    acelerar_porta(-1 * VELOCIDADE_ATUAL, 3000)
    while CL.value() == PRETO:
        acelerar_ajustando(-1 * VELOCIDADE_ATUAL, -1 * VELOCIDADE_ATUAL)
    acelerar(-1 * VELOCIDADE_ATUAL, 2000)
    sleep(30)
    final = False
    quant_bonecos = 0
...
def eh_rampa():
    # Busca identificar se chegou na rampa
    global quant_bonecos, final, sentido, quant_idas
    ...
    if CL.value() == AZUL:
        ...
        if CL.value() == VERDE:
            if quant_idas == 0:
                ...
                acelerar(VELOCIDADE_ATUAL, 2500)
                ...
                final = True
                acelerar(VELOCIDADE_ATUAL, 4500)
            return
    else:
        return
...
def percurso():
    ...
    while True:
        ...
        if quant_bonecos == 0 and sentido == "INDO":
            dist = PROX3.value()
